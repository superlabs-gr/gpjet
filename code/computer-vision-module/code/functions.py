...
def Features_Extraction_y_axis(img,d_cal_factor,stride):
    ...
    for row in range(stride,img.shape[0],stride):
        try:
            ...
            area = stride * d_cal_factor * (diameter + (stride * d_cal_factor / 2) * (
                        math.atan(angle_l * math.pi / 180) + math.atan(angle_r * math.pi / 180)))
            ...
            #        I = img[i, :].tolist()
            #        a = I.index(255)
            #        b = len(I) - 1 - I[::-1].index(255)
            #    except:
            #        a = 0
            #        b = 0
            #    else:
            #        area = area + (b - a) * d_cal_factor ** 2
        areas.append(area)
